...
func Receiver(k float64, cfg config.Config, logFile *os.File, wg *sync.WaitGroup) {
	...
	if err != nil {
		...
		fmt.Errorf(logfileName, "Не удалось подключиться к серверу: %v\n", err)
		os.Exit(1)
	}
	...
	for {
		...
		if statistics.DetectAnomaly(entry.Frequency, k) {
			r.Create(*entry)
			log.Printf("Received Data: Session ID: %s, Frequency: %f, Timestamp: %d", entry.SessionId, entry.Frequency, entry.Timestamp)
		}
		statistics.Update(entry.Frequency)
		log.Printf("Not added to database %f\n", entry.Frequency)
	}
}
