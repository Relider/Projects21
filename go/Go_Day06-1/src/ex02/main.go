...
const (
	...
	FAVICON_PATH     string = "/images/logo.png"
)
...
func main() {
	...
	cfg, err := getCfg()
	URL := fmt.Sprintf(`postgres://%s:%s@localhost:5432/Day06`, cfg.dbUser, cfg.dbPass)
	if err != nil {
...
func (a *App) adminAddArticleHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		err = render.RenderAddArticleForm(w, 0, http.StatusOK)
		if err != nil {
		...
		a.DB.GetTotalNumOfArticles()
		err = render.RenderAddArticleForm(w, id, http.StatusCreated)
		if err != nil {
...
func (a *App) adminLogInHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		err := render.RenderAdminLogInForm(w)
		if err != nil {
			log.Println(err.Error())
...
func (a *App) getArticleHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = render.RenderArticle(w, article, pageNum)
	if err != nil {
...
func (a *App) getArticlesHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = render.RenderIndexArticles(w, articles, intPageNum, (a.DB.TotalArticles-1)/PAGE_LIMIT+1)
	if err != nil {
...
func RespondWithError(w http.ResponseWriter, message string, code int) {
	err := render.RenderError(w, code, message)
	if err != nil {
